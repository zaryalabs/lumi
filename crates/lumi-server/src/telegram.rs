...
const DEFAULT_BOT_SCOPE: &str = "lumi-default";
const MAX_BOT_RESPONSE_BYTES: usize = 2 * 1024 * 1024;
...
impl TelegramService {
    pub(crate) fn from_env(pool: PgPool, imports: Arc<ImportService>) -> Self {
        Self {
            pool,
            imports,
            bot_scope: std::env::var("LUMI_TELEGRAM_BOT_SCOPE")
                .unwrap_or_else(|_| DEFAULT_BOT_SCOPE.to_owned()),
            bot_username: std::env::var("LUMI_TELEGRAM_BOT_USERNAME").ok(),
        }
    }
...
pub(crate) async fn run_long_poll(
    service: Arc<TelegramService>,
    bot_token: &str,
) -> Result<(), TelegramServiceError> {
    if bot_token.trim().is_empty() || bot_token.len() > 256 {
        return Err(TelegramServiceError::Unavailable);
    }
    let mut runner_lock = service.pool.acquire().await.map_err(storage_error)?;
    let lock_acquired: bool =
        sqlx::query_scalar("SELECT pg_try_advisory_lock(hashtextextended($1, 3))")
            .bind(&service.bot_scope)
            .fetch_one(&mut *runner_lock)
            .await
            .map_err(storage_error)?;
    if !lock_acquired {
        tracing::error!(bot_scope = %service.bot_scope, "Telegram runner is already active");
        return Err(TelegramServiceError::Unavailable);
    }
    ...
    let mut fetch_backoff = std::time::Duration::from_secs(1);
    let mut processing_backoff = std::time::Duration::from_secs(1);
    loop {
        let fetched = tokio::select! {
            signal = tokio::signal::ctrl_c() => {
                signal.map_err(|_| TelegramServiceError::Unavailable)?;
                return Ok(());
            }
            result = get_updates(&client, bot_token, offset) => result,
        };
        let updates = match fetched {
            Ok(updates) => {
                fetch_backoff = std::time::Duration::from_secs(1);
                updates
            }
            Err(LongPollError::RateLimited(delay)) => {
                sleep_or_shutdown(delay).await?;
                continue;
            }
            Err(LongPollError::Transient) => {
                sleep_or_shutdown(fetch_backoff).await?;
        ...
        for api_update in updates {
            let update_id = api_update.update_id;
            let Some(update) = api_update.into_domain() else {
                offset = update_id.saturating_add(1);
                processing_backoff = std::time::Duration::from_secs(1);
                continue;
            };
            let reply = match service.handle_update(&update).await {
                ...
                Err(TelegramServiceError::UpdateInProgress)
                | Err(TelegramServiceError::Unavailable) => {
                    sleep_or_shutdown(processing_backoff).await?;
                    processing_backoff = processing_backoff
                        .saturating_mul(2)
                        .min(std::time::Duration::from_secs(30));
                    break;
                }
            };
            if send_reply(&client, bot_token, &reply).await.is_err() {
                sleep_or_shutdown(processing_backoff).await?;
                processing_backoff = processing_backoff
                    .saturating_mul(2)
                    .min(std::time::Duration::from_secs(30));
            ...
            // Advance only after the handler durably records its outcome and
            // the local transport has delivered the deterministic reply.
            offset = update.update_id.saturating_add(1);
            processing_backoff = std::time::Duration::from_secs(1);
        }
    }
...
impl BotApiUpdate {
    fn into_domain(self) -> Option<TelegramUpdate> {
        ...
        Some(TelegramUpdate {
            ...
            forward_origin: message
                .forward_origin
                .as_ref()
                .and_then(redacted_forward_origin),
...
fn redacted_forward_origin(value: &serde_json::Value) -> Option<String> {
    value
        ...
        .or_else(|| value.get("sender_user_name"))
        .or_else(|| {
            value
                .get("sender_user")
                .and_then(|user| user.get("first_name"))
        })
...
async fn get_updates(
    ...
    bot_token: &str,
    offset: i64,
) -> Result<Vec<BotApiUpdate>, LongPollError> {
    let url = bot_api_url(
        bot_token,
        "getUpdates",
        &[("offset", offset.to_string()), ("timeout", "25".to_owned())],
    )
    .ok_or(LongPollError::Transient)?;
    let response = client
        .get(url)
        .send()
        .await
        .map_err(|_| LongPollError::Transient)?;
    let status = response.status();
    if status.is_redirection() {
        return Err(LongPollError::Transient);
    }
    let bytes = read_bot_response(response).await?;
    let envelope: BotApiEnvelope<Vec<BotApiUpdate>> =
        serde_json::from_slice(&bytes).map_err(|_| LongPollError::Transient)?;
    if status.as_u16() == 429 {
        let seconds = envelope
            ...
            .unwrap_or(1)
            .min(60);
        return Err(LongPollError::RateLimited(std::time::Duration::from_secs(
            seconds,
        )));
    }
...
async fn read_bot_response(mut response: reqwest::Response) -> Result<Vec<u8>, LongPollError> {
    if response
        .content_length()
        .is_some_and(|length| length > MAX_BOT_RESPONSE_BYTES as u64)
    {
        return Err(LongPollError::Transient);
    }
    ...
    while let Some(chunk) = response
        ...
        .map_err(|_| LongPollError::Transient)?
    {
        if bytes.len().saturating_add(chunk.len()) > MAX_BOT_RESPONSE_BYTES {
            return Err(LongPollError::Transient);
        }
...
async fn send_reply(
    ...
    let url = bot_api_url(
        ...
        "sendMessage",
        &[
            ("chat_id", reply.chat_id.to_string()),
            ("text", reply.text.clone()),
        ],
    )
    .ok_or(TelegramServiceError::Unavailable)?;
    let response = client
        .post(url)
        .send()
        .await
...
fn bot_api_url(token: &str, method: &str, query: &[(&str, String)]) -> Option<url::Url> {
    let mut url = url::Url::parse(&format!("https://api.telegram.org/bot{token}/{method}")).ok()?;
    url.query_pairs_mut()
        .extend_pairs(query.iter().map(|(key, value)| (*key, value.as_str())));
    Some(url)
}

async fn sleep_or_shutdown(duration: std::time::Duration) -> Result<(), TelegramServiceError> {
    tokio::select! {
        signal = tokio::signal::ctrl_c() => signal.map_err(|_| TelegramServiceError::Unavailable),
