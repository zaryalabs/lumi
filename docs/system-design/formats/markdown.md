...
Главное решение: Markdown не получает отдельный renderer. Markdown
парсится в AST, нормализуется в `ReadingDocument`, а отображение выполняется
общим reader contract из [`../reader-architecture.md`](../reader-architecture.md).
...
Markdown AST превращается в `ReadingDocument`.
...
- **Reader.** Markdown выдает `ReadingDocument`; reader отвечает за paginated
