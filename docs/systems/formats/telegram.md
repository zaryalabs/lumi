...
бот принимает данные, backend создает `Material`, `DocumentRevision` and
Normalized Content Package, а reader отображает результат через общий reader
contract.

## Активный composite baseline

Текущий реализованный срез принимает в private chat обычный или пересланный
текст/caption, Telegram `photo`, альбомы с `media_group_id` и до восьми
публичных HTTP(S)-ссылок. Один пост или один альбом создаёт один материал:
Telegram-вводная идёт первой секцией, затем следуют отдельные web-секции в
порядке ссылок. Исходный URL из вводной не удаляется.

Видео, video note, animation/GIF, audio, voice, document/file и sticker не
скачиваются. Caption и ссылки рядом с ними сохраняются; update только с таким
вложением отклоняется без material. Ограничения: до 10 фото, 10 MiB на фото,
30 MiB фото на материал и три одновременных web fetch.

Transport — встроенный long polling из ADR 0012. Listener ограничивается typed
parsing, private-chat boundary и durable admission. Download, web capture,
normalization и publication выполняет общий import worker. Подробное решение о
source ref, partial failures и media group recovery принято в
[ADR 0013](../../adr/0013-telegram-composite-import.md).
...
Следующий список описывает долгосрочную поверхность формата. Активный срез
ограничен правилами composite baseline выше.
...
- `received` - update принят встроенным long-polling transport.
...
### Нормализация в Normalized Content Package

Telegram content превращается в Normalized Content Package and reader-facing
`ReadingDocument`:
...
Telegram Bot API update
  -> embedded long polling transport
  -> TelegramIdentity
...
- `lumi-server` поднимает long polling supervisor поверх `teloxide-core` во
  всех deployment modes;
- bot token устанавливается через аутентифицированные настройки UI;
- listener и Axum используют общий graceful shutdown;
- transport преобразует типы `teloxide` в существующий durable update handler;
- webhook отложен до multi-replica deployment и описан в ADR 0012.

### Pipeline обработки

1. Получить typed update из встроенного `teloxide` transport.
...
8. Worker скачивает разрешённые `photo` через runtime-managed `getFile` client.
9. Worker раскрывает уникальные web-ссылки через bounded raw fetch.
10. Сохранять каждый готовый artifact и обновлять source ref до следующей стадии.
11. Построить multi-unit Normalized Content Package с partial diagnostics.
12. Атомарно создать один `DocumentRevision`, resource manifest и sync changes.
13. Доставить материал на web/desktop/mobile через обычный sync.
...
- **Reader.** Telegram normalizer выдает Normalized Content Package;
...
- [`teloxide-core` crate](https://docs.rs/teloxide-core/latest/teloxide_core/)
