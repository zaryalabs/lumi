...
Web routes сохраняют typed `LearningOrigin` (`Library`, `Challenges` или
material/source) и передают его в Reader. Back action всегда подписан реальным
destination и возвращает в исходную session/surface. Material card не
показывает ручное `Учиться`, пока нет валидного `source_id`; честный вход идёт
из completion offer либо существующего material learning source.
