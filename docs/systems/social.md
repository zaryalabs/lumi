...
не технический sync namespace, а страница и среда конкретного субъекта в Lumi.
Доступные `CommunitySpace` собраны в primary destination `Пространства`;
контекстные входы из Reader, Search и карточки материала сохраняются.
...
коллаборативной активностью; в Web shell эта группа называется
`Пространства`.
...
shared highlights, activity и общий chat. В shell находится permission-aware
список пространств, в которых пользователь состоит. Сначала показываются
memberships и материалы/обсуждение выбранного Space; создание, members,
invites и управление открываются вторично.

Matched material предлагает `Открыть свою копию` и ведёт в личный Reader с
typed Community origin. Social search route сохраняет identity материала,
сообщения или highlight и после загрузки переводит фокус к точному объекту.
...
- Пользователь открывает `Пространства` и переходит в одно из сообществ, в
  которых участвует.
