...
Первые срезы строим не как MVP с временной архитектурой, а как минимальный
вертикальный остов полной системы:
...
4. **S3 Android Reader.** Android client на той же модели: локальная реплика,
   EPUB reading, sync, базовые highlights/notes.

После S1-S3 продукт развивается функционально через регистр: новые форматы,
поиск, KB, AI, learning, social, Obsidian, plugins and additional platforms.

Главное ограничение: первые срезы могут быть узкими по user-facing scope, но не
должны создавать альтернативную архитектуру, которую потом придется выбрасывать.
...
- Reader: постраничное или page-like чтение, TOC/navigation, progress,
  basic reader settings.
- Базовые annotations:
  - highlight text selection;
  - note attached to highlight или block/location;
  - list of notes/highlights for current material;
...
- **No format-specific reader.** EPUB import must produce `Material`,
  `DocumentRevision`, Normalized Content Package and `ReadingDocument`. Reader
  must not render EPUB XHTML/CSS as the product model.
...
- Minimal seed phrase or seed-derived auth prototype, behind the final auth
  boundary. If exact OPAQUE/PAKE choice is not ready, record ADR and keep the
...
- Advanced gestures, TTS and accessibility polish beyond baseline usable reader.
...
1. **Search and export hardening.** In-document/global search, better annotation
   export, more EPUB fixtures.
...
7. **Plugin/runtime path.** Start with first-party plugin blocks, then move to
   third-party runtime only after the contracts are stable.
