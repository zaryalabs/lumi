...
`DocumentRevision` and Normalized Content Package, а reader строит
`ReadingDocument` через общий reader contract из
...
FB2 body нормализуется в Normalized Content Package. `ReadingDocument`
...
13. Создать `DocumentRevision`, Normalized Content Package, `ReadingDocument`
    view, source map и import issues.
...
- **Reader.** FB2 importer выдает Normalized Content Package; `ReadingDocument`
